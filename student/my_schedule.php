...
// Fetch student info including class_year
$student_sql = "SELECT * FROM students WHERE user_id = '$student_user_id'";
$student_result = $conn->query($student_sql);
$student = $student_result->fetch_assoc();

$class_year = $student['class_year'];

// Fetch schedules that match the student's class_year
$schedules_sql = "SELECT sch.*, t.firstname, t.lastname
                  FROM schedules sch
                  LEFT JOIN teachers t ON sch.teacher_id = t.id
                  WHERE sch.class_year = '$class_year'
...
<div class="dashboard-container">
    ...
    <div class="main-content">
        ...
        <div class="paper-container">
            ...
            <div class="bar-title bar-red">2ND SEMESTER AY 2025-2026</div> <!-- User might want dynamic sem/year later -->
            <div class="bar-title bar-cyan"><?php echo htmlspecialchars($class_year ? $class_year : 'NO CLASS ASSIGNED'); ?></div>
                        ...
                        <?php for ($i = 0; $i < $row_count; $i++): ?>
                            <tr>
                                ...
                                <?php if (isset($day_items[$i])): 
                                    $item = $day_items[$i];
                                ?>
                                    <td class="time-cell"><?php echo htmlspecialchars($item['time_start']); ?></td>
                                    <td class="subject-cell"><?php echo htmlspecialchars($item['subject']); ?></td>
                                    <td><?php echo htmlspecialchars($item['firstname'] . ' ' . $item['lastname']); ?></td>
                                    <td><?php echo htmlspecialchars($item['room'] ?: '-'); ?></td>
                                <?php else: ?>
                                    <!-- Empty Row if no schedule (optional, but requested layout often implies specific slots. 
                                         I'll just leave empty cells if no class for the day, or just one empty row) -->
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
