...
$schedules_sql = "SELECT id, class_year, subject, day, time_start 
                  FROM schedules 
                  WHERE teacher_id = '$teacher_id' 
                  ORDER BY class_year, subject";
$schedules_result = $conn->query($schedules_sql);

// Fetch students and grades for selected schedule
$students = [];
if ($selected_schedule) {
    // Get class_year from selected schedule
    $sched_info = $conn->query("SELECT class_year, subject FROM schedules WHERE id = '$selected_schedule'")->fetch_assoc();
    
    if ($sched_info) {
        $class_year = $sched_info['class_year'];
