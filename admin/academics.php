            ...
            <form method="POST">
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" name="course_code" id="addCourseCode" class="form-control" placeholder="e.g., BSIT" required style="text-transform: uppercase;">
                    </div>
                    <div class="form-group">
                        <label>Course Name *</label>
                        <input type="text" name="course_name" id="addCourseName" class="form-control" placeholder="Bachelor of Science in..." required>
                    </div>
                    <div style="display: flex; align-items: flex-end;">
                        <button type="submit" name="save_course" class="btn btn-primary" style="height: 42px;">Add Course</button>
                    </div>
                </div>
            </form>
        </div>
            ...
            <form method="POST">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="strand_code" id="addStrandCode" class="form-control" placeholder="e.g., STEM, CSS" required style="text-transform: uppercase;">
                    </div>
                    <div class="form-group">
                        <label>Strand Name *</label>
                        <input type="text" name="strand_name" id="addStrandName" class="form-control" placeholder="Science, Technology..." required>
                    </div>
                    <div style="display: flex; align-items: flex-end;">
                        <button type="submit" name="save_strand" class="btn btn-success" style="height: 42px;">Add Strand</button>
                    </div>
                </div>
            </form>
        </div>
                        ...
                        <?php if ($courses->num_rows > 0): ?>
                            <?php while($c = $courses->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><a href="?delete_course=<?php echo $c['id']; ?>" class="btn btn-danger" style="padding: 4px 8px; font-size: 0.75rem;" onclick="return confirm('Delete?');">Delete</a></td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="3" style="text-align: center; color: #888;">No courses found.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if($s['description'] == 'TVL'): ?>
                                            <span style="background: #e67e22; color: white; padding: 2px 8px; border-radius: 4px; font-size: 0.8rem;">TVL</span>
                                        <?php else: ?>
                                            <span style="background: #27ae60; color: white; padding: 2px 8px; border-radius: 4px; font-size: 0.8rem;">Academic</span>
                                        <?php endif; ?>
                                    </td>
                                    <td><a href="?delete_strand=<?php echo $s['id']; ?>" class="btn btn-danger" style="padding: 4px 8px; font-size: 0.75rem;" onclick="return confirm('Delete?');">Delete</a></td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="4" style="text-align: center; color: #888;">No strands found.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <form method="POST" id="sectionForm">
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select name="category" id="categorySelect" class="form-control" required onchange="toggleItems()">
                ...
                <div id="sectionPreview" style="background: #e3f2fd; padding: 15px; border-radius: 8px; margin-bottom: 20px; display: none; text-align: center; border: 1px dashed #2196f3;">
                ...
                <tbody>
                    <?php if ($result_sections->num_rows > 0): ?>
                        <?php while($section = $result_sections->fetch_assoc()): ?>
                            <tr>
                                <td style="font-weight: 600; color: #0056b3;"><?php echo htmlspecialchars($section['code']); ?> - <?php echo htmlspecialchars($section['name']); ?></td>
                                <td><?php echo htmlspecialchars($section['grade_level']); ?></td>
                                <td>
                                    <span style="padding: 3px 10px; border-radius: 12px; font-size: 0.8rem; background: <?php echo $section['type'] == 'College' ? '#3498db' : '#2ecc71'; ?>; color: white;">
                                        <?php echo htmlspecialchars($section['type']); ?>
                                    </span>
                                </td>
                                <td>
                                    <span style="padding: 3px 10px; border-radius: 12px; font-size: 0.8rem; background: #f39c12; color: white;">
                                        <?php echo htmlspecialchars($section['block']); ?>
                                    </span>
                                </td>
                                <td>
                                    <a href="?delete_section=<?php echo $section['id']; ?>" class="btn btn-danger" style="padding: 5px 10px; font-size: 0.85rem;" onclick="return confirm('Delete this section?');">Delete</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="5" style="text-align: center; color: #888; padding: 30px;">No sections found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
